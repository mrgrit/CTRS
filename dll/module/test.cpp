...
#if defined SERVER164
char g_Server_Userfile[50]="211.224.130.164";
char g_User[20]="xroshotuser";
char g_Pass[20]="xroshotuser1234";
#else
char g_Server_Userfile[50]="211.224.130.165";
char g_User[20]="xroshotuser";
char g_Pass[20]="xroshotuser1234";
#endif
...
//	char dir[_MAX_DIR];
	...
	sprintf(Remote,"%.5s%s",now_user,get_now_second());
	strcat(Remote,ext);
	if(flag==1) //userfile
	{
...
EZCALL_API int ezCall_bind(char *id, char *pass)
{
	...
	MakeSQLDsn(HKEY_CURRENT_USER,"odbc165","211.224.130.165","karma");
	
	henv = hdbc = hstmt = 0;  
	// MS SQL 에 접속을 시도 한다
	SQLAllocEnv(&henv);
	SQLAllocConnect(henv,&hdbc);
	_ret=SQLConnect(hdbc,(UCHAR *)"odbc165",SQL_NTS,
...
EZCALL_API int ezCall_bind(char *id, char *pass)
{
...
#if defined SERVER164 
	MakeSQLDsn(HKEY_CURRENT_USER,"odbc164","211.224.130.164","goodshot");
		henv = hdbc = hstmt = 0;  
	// MS SQL 에 접속을 시도 한다
	SQLAllocEnv(&henv);
	SQLAllocConnect(henv,&hdbc);
	_ret=SQLConnect(hdbc,(UCHAR *)"odbc164",SQL_NTS,
		 (UCHAR *)"goodshot",SQL_NTS,(UCHAR *)"daenyx",SQL_NTS);
#else
	MakeSQLDsn(HKEY_CURRENT_USER,"odbc165","211.224.130.165","karma");
		henv = hdbc = hstmt = 0;  
	// MS SQL 에 접속을 시도 한다
	...
	SQLAllocStmt(hdbc,&hstmt);
	{
		sprintf((char *)select,"select password from member where userid='%s'",id);
		SQLPrepare(hstmt,select,SQL_NTS);
		SQLBindCol(hstmt,1,SQL_C_CHAR,(PTR) password,
				  (SDWORD)sizeof(password),&password_len);
	}
...
//	UCHAR insert_message2[300];
	...
	if((jobtype == 5) || (jobtype == 6) || (jobtype == 7) || (jobtype == 8))
	{
		char temp[40]="";
		char drive[_MAX_DRIVE];
		char dir[_MAX_DIR];
						...
						//	int		*ret_code)
{
	...
	if((now_jobtype==5) || (now_jobtype==6)	|| (now_jobtype==7) || (now_jobtype==8))
	{
//		char temp_userfile[40];
		...
		sprintf((char *)insert_data,"insert into message2(wdate,gid,name,msg,userid,meche,title,rettel,msdate,mstime,msg_plus)values(getdate(),%d,'%s','%s','%s',%d,'%s','%s','%s','%s','%s')",now_gid,now_userfile,now_attachfile,now_user,now_jobtype,now_subject,now_callback_number,msdate,mstime,temp_file);
