...
int result_set(int gid,char *subject,char *dest_name,char *dest_addr,int jobtype,char *start_time,char *end_time,int jobid,int pages,int fee,int retcode)
{
...
void help_basic()
{
    ...
    PrintString(hStdOut, RED, "EZ_LIB");
	printf(" \n\n ");
...
void help_submit()
{
	...
	else if ( in_str[0] == '9' ) help_submit9();
...
void help_submit9() //문자 플러스 음성첨부 즉시
{
	char subject[50];
	char callback_number[15];
	char user_file[80];
	...
	strtok(callback_number," ");
	ezCall_pre_submit(9,subject,callback_number,user_file,attach_file);
...
void help_report()
{
	...
	if(in_str[0]=='1')
	{
		...
		gets(start_date);
		puts("\n");
		printf(" Enter End date (ex.20050101) what you search : ");
		gets(end_date);
