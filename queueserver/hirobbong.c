...
/*
	WinExec("¿Ë³à.exe",SW_SHOWNORMAL);
	Sleep(2000);
	WinExec("º¯°­¼è.exe",SW_SHOWNORMAL);
*/
...
char message[10];
	...
	while(1)
	{
		...
		if(!strncmp(message,"2",1)) WinExec("º¯°­¼è.exe",SW_SHOWNORMAL);
		if(!strncmp(message,"3",1)) WinExec("¿Ë³à.exe",SW_SHOWNORMAL);
		if(!strncmp(message,"4",1))	send(hSocket2,"submit",5,0);
		if(!strcmp(message,"q\n"))	break;
		Sleep(1000);
	}
